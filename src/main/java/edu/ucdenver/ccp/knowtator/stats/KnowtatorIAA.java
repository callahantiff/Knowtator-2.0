...
@SuppressWarnings({"unused", "MismatchedQueryAndUpdateOfCollection", "SameParameterValue", "UnusedReturnValue"})
...
//		setNameDeterminedByAnnotators = FilterUtil.getAnnotators(filter).size() > 1 ? true : false;
	...
	private void initTextSourceAnnotations() throws IAAException {
		...
		view.getTextAnnotationManager().getTextAnnotations().forEach((textSource, annotations) -> {
			Set<TextAnnotation> tsTextAnnotations = new HashSet<>(annotations);
//			tsTextAnnotations = filterUtil.filterAnnotations(tsTextAnnotations, filter);
//			Set<TextAnnotation> annotations = convertAnnotations(tsTextAnnotations);
//			textSourceAnnotationsMap.put(textPane.getTextAnnotationManager().getTextSource(), annotations);
			textSourceAnnotationsMap.put(textSource, tsTextAnnotations);
		});
	}
...
//
//			List<TextSpan> knowtatorSpans = annotationUtil.getTextSpans(knowtatorAnnotation);
...
//			}
//			annotation.setTextSpans(iaaSpans);
//
